...
#include "yai/cli/render.h"
#include "yai/cli/display_map.h"
#include "yai/cli/color.h"
#include "yai/cli/style_map.h"
...
static int is_workspace_command(const yai_sdk_reply_t *out, const yai_render_opts_t *opts)
{
  ...
  return strncmp(cid, "yai.workspace.", 14) == 0;
}
...
static void print_runtime_capabilities(FILE *stream, const cJSON *data)
{
  ...
  int knowledge_ready = 0, knowledge_known = 0;
  ...
  knowledge_known = json_bool_value_path(caps, "knowledge.ready", &knowledge_ready);
  ...
  print_kv(stream, "Workspace selected", selected_known ? semantic_bool(selected, "yes", "no") : "unknown");
  print_kv(stream, "Workspace bound", bound_known ? semantic_bool(bound, "yes", "no") : "unknown");
  ...
  print_kv(stream, "Knowledge", semantic_capability_ready(knowledge_known, knowledge_ready));
...
static void print_operator_runtime_surface(FILE *stream, const yai_sdk_reply_t *out)
{
  ...
  print_section(stream, "Workspace binding");
  print_kv(stream, "Workspace selected", workspace_selected);
  print_kv(stream, "Workspace bound", workspace_bound);
  ...
  print_kv(stream, "Knowledge", "unknown");
...
static void print_ws_current(FILE *stream, const cJSON *data)
{
  ...
  print_title(stream, "Workspace current");
...
static void print_ws_status(FILE *stream, const cJSON *data)
{
  ...
  print_title(stream, "Workspace status");
...
static void print_ws_domain(FILE *stream, const cJSON *data, const char *title)
{
  ...
  print_kv(stream, "Workspace", json_string_path(data, "workspace_id", "—"));
...
static void print_ws_inspect(FILE *stream, const cJSON *data)
{
  ...
  print_title(stream, "Workspace inspect");
  print_section(stream, "Identity");
...
static void print_ws_policy_effective(FILE *stream, const cJSON *data)
{
  ...
  attach_count_buf[0] = '\0';
  if (cJSON_IsNumber(attach_count)) snprintf(attach_count_buf, sizeof(attach_count_buf), "%d", attach_count->valueint);
  print_title(stream, "Workspace policy effective");
  print_kv(stream, "Workspace", json_string_path(data, "workspace_id", "—"));
...
static void print_ws_debug_resolution(FILE *stream, const cJSON *data)
{
  ...
  print_title(stream, "Workspace debug resolution");
  print_kv(stream, "Workspace", json_string_path(data, "workspace_id", "—"));
...
static void print_ws_run(FILE *stream, const yai_render_opts_t *opts, const cJSON *data)
{
  ...
  print_title(stream, "Workspace run");
  print_section(stream, "Execution");
  print_kv(stream, "Action", action);
  print_kv(stream, "Workspace", json_string_path(data, "ws_id", "—"));
...
static void print_ws_query_result(FILE *stream, const cJSON *data, const char *title)
{
  ...
  print_title(stream, title ? title : "Workspace query");
...
static void print_ws_knowledge_composed(FILE *stream, const cJSON *data)
{
  if (!stream || !data) return;
  print_title(stream, "Workspace knowledge");
  print_section(stream, "Context");
  print_kv(stream, "Workspace", json_string_path(data, "identity.workspace_id", "—"));
  ...
  print_section(stream, "Knowledge");
  print_kv(stream, "Ready", json_bool_path(data, "runtime_capabilities.knowledge.ready", "yes", "no"));
  print_kv(stream, "Transient authoritative", json_bool_path(data, "runtime_capabilities.knowledge.transient_authoritative", "yes", "no"));
...
static int render_source_data_human(const yai_sdk_reply_t *out, const yai_render_opts_t *opts, FILE *stream)
{
  ...
  if (cid && strcmp(cid, "yai.source.attach") == 0) {
    ...
    print_section(stream, "Binding");
    print_kv(stream, "Workspace", json_string_path(data, "workspace_id", "—"));
    print_kv(stream, "Owner workspace", json_string_path(data, "owner_workspace_id", "—"));
    print_kv(stream, "Source node", json_string_path(data, "source_node_id", "—"));
...
static int render_workspace_data_human(const yai_sdk_reply_t *out, const yai_render_opts_t *opts, FILE *stream)
{
  ...
  if (cid && strcmp(cid, "yai.workspace.prompt_context") == 0) {
  ...
  if (cid && strcmp(cid, "yai.workspace.current") == 0) print_ws_current(stream, data);
  else if (cid && strcmp(cid, "yai.workspace.status") == 0) print_ws_status(stream, data);
  else if (cid && strcmp(cid, "yai.workspace.domain_get") == 0) print_ws_domain(stream, data, "Workspace domain");
  else if (cid && strcmp(cid, "yai.workspace.domain_set") == 0) print_ws_domain(stream, data, "Workspace domain updated");
  else if (cid && strcmp(cid, "yai.workspace.inspect") == 0) print_ws_inspect(stream, data);
  else if (cid && starts_with(cid, "yai.workspace.knowledge.")) print_ws_knowledge_composed(stream, data);
  else if (cid && strcmp(cid, "yai.workspace.policy_effective") == 0) print_ws_policy_effective(stream, data);
  else if (cid && strcmp(cid, "yai.workspace.policy_dry_run") == 0) {
    print_title(stream, "Workspace policy dry-run");
    print_kv(stream, "Workspace", json_string_path(data, "workspace_id", "—"));
  ...
  else if (cid && strcmp(cid, "yai.workspace.policy_attach") == 0) {
    print_title(stream, "Workspace policy attach");
    print_kv(stream, "Workspace", json_string_path(data, "workspace_id", "—"));
    print_kv(stream, "Object", json_string_path(data, "object_id", "—"));
  ...
  else if (cid && strcmp(cid, "yai.workspace.policy_activate") == 0) {
    print_title(stream, "Workspace policy activate");
    print_kv(stream, "Workspace", json_string_path(data, "workspace_id", "—"));
    print_kv(stream, "Object", json_string_path(data, "object_id", "—"));
    print_kv(stream, "State", json_string_path(data, "attachment_state", "attached_active"));
    print_kv(stream, "Activation", json_string_path(data, "activation_state", "active"));
  ...
  else if (cid && strcmp(cid, "yai.workspace.policy_detach") == 0) {
    print_title(stream, "Workspace policy detach");
    print_kv(stream, "Workspace", json_string_path(data, "workspace_id", "—"));
    print_kv(stream, "Object", json_string_path(data, "object_id", "—"));
    print_kv(stream, "Validity", json_string_path(data, "attachment_valid", "true"));
  ...
  else if (cid && strcmp(cid, "yai.workspace.debug_resolution") == 0) print_ws_debug_resolution(stream, data);
  else if (cid && strcmp(cid, "yai.workspace.run") == 0) print_ws_run(stream, opts, data);
  else if ((cid && starts_with(cid, "yai.workspace.graph.")) ||
           (cid && starts_with(cid, "yai.workspace.db.")) ||
           (cid && starts_with(cid, "yai.workspace.data.")) ||
           (cid && starts_with(cid, "yai.workspace.knowledge.")) ||
           (cid && strcmp(cid, "yai.workspace.query") == 0) ||
           (cid && strcmp(cid, "yai.workspace.events.tail") == 0) ||
           (cJSON_IsString(cJSON_GetObjectItemCaseSensitive(data, "type")) &&
            strcmp(cJSON_GetObjectItemCaseSensitive(data, "type")->valuestring, "yai.workspace.query.result.v1") == 0)) {
    const char *title = "Workspace query";
    if (cid && starts_with(cid, "yai.workspace.graph.")) title = "Workspace graph";
    else if (cid && starts_with(cid, "yai.workspace.db.")) title = "Workspace db";
    else if (cid && starts_with(cid, "yai.workspace.data.")) title = "Workspace data";
    else if (cid && starts_with(cid, "yai.workspace.knowledge.")) title = "Workspace knowledge";
    else if (cid && strcmp(cid, "yai.workspace.events.tail") == 0) title = "Workspace data";
    else if (cid && strcmp(cid, "yai.workspace.query") == 0) {
      const char *family = json_string_path(data, "query_family", "");
      if (starts_with(family, "graph")) title = "Workspace graph";
      else if (starts_with(family, "transient") || starts_with(family, "memory")) title = "Workspace knowledge";
      else if (starts_with(family, "events") || starts_with(family, "evidence") || starts_with(family, "governance") ||
               starts_with(family, "authority") || starts_with(family, "artifact") || starts_with(family, "enforcement")) title = "Workspace data";
      else if (starts_with(family, "workspace")) title = "Workspace db";
    }
    print_ws_query_result(stream, data, title);
  }
  else if (cid && (strcmp(cid, "yai.workspace.set") == 0 || strcmp(cid, "yai.workspace.switch") == 0)) {
    print_title(stream, strcmp(cid, "yai.workspace.switch") == 0 ? "Workspace switched" : "Workspace set");
    print_kv(stream, "Workspace", json_string_path(data, "workspace_id", "—"));
...
int yai_render_exec_short(const yai_sdk_reply_t *out, int rc, const yai_render_opts_t *opts)
{
  ...
  if (lifecycle_command) {
    ...
    if (strcmp(out->command_id, "yai.lifecycle.up") == 0) {
      ...
      print_kv(stream, "Workspace binding", "unknown (run: yai ws status)");

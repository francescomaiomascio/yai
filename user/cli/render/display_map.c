...
void yai_display_from_reply(const yai_sdk_reply_t *reply, yai_display_result_t *out)
{
  ...
  if (strcmp(reply->code, "BAD_ARGS") == 0 || strcmp(reply->code, "INVALID_TARGET") == 0) {
    ...
    if (strcmp(reply->reason, "family_not_found") == 0) {
      snprintf(out->detail, sizeof(out->detail), "Invalid workspace domain arguments: unknown control family.");
      ...
      snprintf(out->detail, sizeof(out->detail), "Invalid workspace domain arguments: unknown specialization.");
      ...
      snprintf(out->detail, sizeof(out->detail), "Invalid workspace domain arguments: specialization does not belong to selected family.");
      ...
      snprintf(out->detail, sizeof(out->detail), "No active workspace selected for runtime execution.");
      ...
      snprintf(out->detail, sizeof(out->detail), "Policy object is not attached to current workspace.");
      snprintf(out->hint, sizeof(out->hint), "Run: yai ws policy effective");
    } else if (strcmp(reply->reason, "attachment_limit_reached") == 0) {
      snprintf(out->detail, sizeof(out->detail), "Workspace attachment limit reached.");
      ...
      snprintf(out->detail, sizeof(out->detail), "No workspace is selected for this command.");
      snprintf(out->hint, sizeof(out->hint), "Run: yai ws set <ws-id>");
    } else if (strcmp(reply->reason, "workspace_context_invalid") == 0) {
      snprintf(out->detail, sizeof(out->detail), "Workspace is selected but binding state is invalid.");
      snprintf(out->hint, sizeof(out->hint), "Run: yai ws status");
    } else if (starts_with(reply->reason, "workspace_postcondition_")) {
      snprintf(out->detail, sizeof(out->detail), "Workspace binding post-conditions failed: %s", reply->reason);
  ...
  if (strcmp(reply->code, "SERVER_UNAVAILABLE") == 0) {
    ...
    if (starts_with(reply->command_id, "yai.workspace.")) {
      snprintf(out->detail, sizeof(out->detail), "Runtime control plane is unreachable before workspace checks.");

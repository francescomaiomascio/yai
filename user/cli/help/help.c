...
#include "yai/cli/help.h"

#include "yai/cli/errors.h"
#include "yai/cli/pager.h"
...
static int render_watch_entrypoint_help(strbuf_t *sb)
{
  ...
  sb_appendf(sb, "  inspect            watch inspect command loops (runtime/workspace/catalog/context + projections)\n");
...
static int render_entrypoint_help(const yai_sdk_command_catalog_t *idx, const char *entrypoint, strbuf_t *sb)
{
  ...
  if (strcmp(entrypoint, "ws") == 0) {
    sb_appendf(sb, "ws\n\n");
    sb_appendf(sb, "Workspace commands:\n");
    sb_appendf(sb, "  graph                      workspace graph read surfaces\n");
    sb_appendf(sb, "  db                         workspace data-store/binding read surfaces\n");
    sb_appendf(sb, "  data                       workspace record query surfaces\n");
    sb_appendf(sb, "  knowledge                  workspace knowledge/transient query surfaces\n");
    sb_appendf(sb, "  policy                     workspace governance attach/effective surfaces\n");
    sb_appendf(sb, "  domain                     workspace domain context surfaces\n");
    sb_appendf(sb, "  recovery                   workspace recovery/load/reopen surfaces\n");
    sb_appendf(sb, "  debug                      workspace resolution diagnostics\n");
    sb_appendf(sb, "  query <family>             generic fallback family query\n");
    sb_appendf(sb, "\nLifecycle and context:\n");
    sb_appendf(sb, "  create <ws-id>             create workspace runtime state\n");
    sb_appendf(sb, "  open <ws-id>               open and bind existing workspace\n");
    sb_appendf(sb, "  reset <ws-id>              reset workspace runtime state\n");
    sb_appendf(sb, "  destroy <ws-id>            destroy workspace runtime state\n");
    sb_appendf(sb, "  set <ws-id>                set active workspace binding\n");
    sb_appendf(sb, "  switch <ws-id>             switch active workspace binding\n");
    sb_appendf(sb, "  current                    show current workspace\n");
    sb_appendf(sb, "  status                     show workspace binding/runtime status\n");
    sb_appendf(sb, "  inspect                    show expanded workspace snapshot\n");
    sb_appendf(sb, "  unset                      remove active workspace binding\n");
    sb_appendf(sb, "  clear                      clear active workspace runtime state\n");
    ...
    sb_appendf(sb, "                             set declared workspace context\n");
    sb_appendf(sb, "  run <action> [tokens...]   run a real runtime action in active workspace context\n");
    sb_appendf(sb, "  policy dry-run <object-id> show eligibility/compatibility preview for workspace apply\n");
    sb_appendf(sb, "  policy attach <object-id>  attach governable policy object to active workspace\n");
    sb_appendf(sb, "  policy activate <object-id> activate an already attached governable object\n");
    sb_appendf(sb, "  policy detach <object-id>  detach governable policy object from active workspace\n");
    sb_appendf(sb, "  policy effective           show effective policy summary\n");
    sb_appendf(sb, "  debug resolution           show debug resolution summary\n");
    sb_appendf(sb, "  prompt-context             print compact workspace context payload\n");
    sb_appendf(sb, "  prompt-token               print compact prompt token (e.g. \u25c9 demo)\n");
    sb_appendf(sb, "\nUse: yai help ws <graph|db|data|knowledge|policy|domain|recovery|debug>\n");
    return 0;
  }
  if (strcmp(entrypoint, "source") == 0) {
    sb_appendf(sb, "source\n\n");
    sb_appendf(sb, "Source-plane commands:\n");
    sb_appendf(sb, "  enroll <source-label>      enroll source node/daemon against owner workspace\n");
    sb_appendf(sb, "  attach <source-node-id>    attach source node to owner workspace context\n");
    sb_appendf(sb, "  list                       list source-plane summary for active workspace\n");
    sb_appendf(sb, "  status                     show source-plane health/summary for active workspace\n");
    ...
    sb_appendf(sb, "  edge process binary remains yai-daemon; operator CLI namespace is yai source.\n");
...
static int render_topic_help(const yai_sdk_command_catalog_t *idx, const char *entrypoint, const char *topic, strbuf_t *sb)
{
  ...
  if (strcmp(entrypoint, "ws") == 0) {
    if (strcmp(topic, "graph") == 0) {
      ...
      sb_appendf(sb, "Usage:\n");
      sb_appendf(sb, "  yai ws graph <summary|workspace|governance|decision|evidence|authority|artifact|lineage|recent>\n");
      return 0;
    }
    ...
    if (strcmp(topic, "data") == 0) {
      ...
      sb_appendf(sb, "\nNotes:\n");
      sb_appendf(sb, "  data family uses workspace query substrate with canonical ws grammar.\n");
      return 0;
    }
    if (strcmp(topic, "knowledge") == 0) {
      sb_appendf(sb, "ws knowledge\n\n");
      sb_appendf(sb, "Usage:\n");
      sb_appendf(sb, "  yai ws knowledge <status|transient|memory|providers|context>\n");
      sb_appendf(sb, "\nNotes:\n");
      sb_appendf(sb, "  canonical path: use ws knowledge first.\n");
  ...
  if (strcmp(entrypoint, "source") == 0) {
    ...
    if (strcmp(topic, "attach") == 0) {
      ...
      sb_appendf(sb, "Usage:\n");
      sb_appendf(sb, "  yai source attach <source-node-id> [--scope <workspace|...>] [--constraints-ref <ref>] [--owner-workspace-id <ws-id>] [--ws-id <ws-id>]\n");
      return 0;
    }
  ...
  if (strcmp(entrypoint, "inspect") == 0) {
    if (!(strcmp(topic, "workspace") == 0 || strcmp(topic, "runtime") == 0 ||

...
#include "yai/cli/control_call.h"
#include "yai/cli/lifecycle.h"
#include "yai/cli/color.h"
#include "yai/cli/watch.h"
#include "yai/cli/term.h"
...
static int command_has_ws_flag(int argc, char **argv)
{
  for (int i = 0; i + 1 < argc; i++) {
    ...
    if ((strcmp(argv[i], "--ws-id") == 0 || strcmp(argv[i], "--ws") == 0 ||
         strcmp(argv[i], "--workspace-id") == 0 || strcmp(argv[i], "--workspace") == 0) &&
        argv[i + 1] && argv[i + 1][0]) {
      return 1;
    }
  }
...
static int is_workspace_clear_command(const char *command_id)
{
  if (!command_id || !command_id[0]) return 0;
  return strcmp(command_id, "yai.workspace.clear") == 0;
}
...
static const char *dispatch_runtime_command_id(const char *command_id)
{
  ...
  if (strcmp(command_id, "yai.workspace.db.classes") == 0) return "yai.workspace.query";
  if (strcmp(command_id, "yai.workspace.db.count") == 0) return "yai.workspace.query";
  if (strcmp(command_id, "yai.workspace.db.tail") == 0) return "yai.workspace.events.tail";
  if (strcmp(command_id, "yai.workspace.data.events") == 0) return "yai.workspace.query";
  if (strcmp(command_id, "yai.workspace.data.evidence") == 0) return "yai.workspace.query";
  if (strcmp(command_id, "yai.workspace.data.governance") == 0) return "yai.workspace.query";
  if (strcmp(command_id, "yai.workspace.data.authority") == 0) return "yai.workspace.query";
  if (strcmp(command_id, "yai.workspace.data.artifacts") == 0) return "yai.workspace.query";
  if (strcmp(command_id, "yai.workspace.data.enforcement") == 0) return "yai.workspace.query";
  if (strcmp(command_id, "yai.workspace.knowledge.transient") == 0) return "yai.workspace.inspect";
  if (strcmp(command_id, "yai.workspace.knowledge.memory") == 0) return "yai.workspace.inspect";
  if (strcmp(command_id, "yai.workspace.knowledge.providers") == 0) return "yai.workspace.inspect";
  if (strcmp(command_id, "yai.workspace.knowledge.context") == 0) return "yai.workspace.inspect";
  if (strcmp(command_id, "yai.source.list") == 0) return "yai.workspace.query";
  if (strcmp(command_id, "yai.source.status") == 0) return "yai.workspace.query";
  if (strcmp(command_id, "yai.source.inspect") == 0) return "yai.workspace.query";
  return command_id;
}
    ...
    const char **argv)
{
  ...
  ws = find_arg_value(argc, argv, "--workspace-id");
  if (ws && ws[0]) return ws;
  ws = find_arg_value(argc, argv, "--workspace");
  if (ws && ws[0]) return ws;
    ...
    yai_sdk_reply_t *reply)
{
  ...
  if (strcmp(command_id, "yai.source.enroll") == 0) {
    ...
    if (!ws || !ws[0]) {
      set_source_reply(reply, command_id, "error", "BAD_ARGS", "workspace_not_active",
                       "No workspace selected for source enroll.",
  ...
  if (strcmp(command_id, "yai.source.attach") == 0) {
    ...
    if (!ws || !ws[0]) {
      set_source_reply(reply, command_id, "error", "BAD_ARGS", "workspace_not_active",
                       "No workspace selected for source attach.",
                       "Run: yai ws set <ws-id> or pass --ws-id <ws-id>");
      return YAI_SDK_BAD_ARGS;
    }
    memset(&req, 0, sizeof(req));
    req.workspace_id = ws;
    req.owner_workspace_id = find_arg_value(argc, argv, "--owner-workspace-id");
    req.source_node_id = argv[0];
    req.binding_scope = find_arg_value(argc, argv, "--scope");
    if (!req.binding_scope || !req.binding_scope[0]) req.binding_scope = "workspace";
...
static int operator_runtime_ping(yai_sdk_reply_t *reply, const char *command_id)
{
...

  {
    const char *current_call =
        "{\"type\":\"yai.control.call.v1\",\"target_plane\":\"runtime\",\"command_id\":\"yai.workspace.current\",\"argv\":[]}";
...
static int handle_operator_capability(const yai_porcelain_request_t *req, yai_sdk_reply_t *reply)
{
  ...
  if (strcmp(req->command_id, "yai.operator.doctor.pins") == 0) {
    int law_ok = file_exists("deps/law/registry/commands.v1.json");
    int sdk_ok = file_exists("user/libs/libyai/include/yai/sdk/public.h");
    if (!law_ok || !sdk_ok) {
  ...
  if (strcmp(req->command_id, "yai.operator.doctor.all") == 0) {
    ...
    int pins_ok = file_exists("deps/law/registry/commands.v1.json") &&
                  file_exists("user/libs/libyai/include/yai/sdk/public.h");
    ...
    if (runtime_ok && env_ok && pins_ok) {
      ...
      operator_reply_set(reply, req->command_id, "ok", "OK", "doctor_all_ok",
                         ...
                         ws_ok ? "" : "No current workspace selected; run: yai ws set <ws-id>",
                         "",
                         details);
      return 0;
    }
  ...
  if (strcmp(req->command_id, "yai.operator.verify.law") == 0) {
    if (!file_exists("deps/law/registry/commands.v1.json")) {
  ...
  switch (req.kind) {
    ...
    case YAI_PORCELAIN_KIND_COMMAND:
      ...
      }
      {
        ...
        if (strcmp(req.command_id, "yai.workspace.run") == 0) {
        ...
        if ((strcmp(req.command_id, "yai.source.list") == 0 ||
            ...
            dispatch_command_id && strcmp(dispatch_command_id, "yai.workspace.query") == 0) {
        ...
        if (strcmp(req.command_id, "yai.lifecycle.up") == 0 ||
          ...
          if (sdk_rc == 0) {
            ...
            if (is_workspace_clear_command(req.command_id)) {

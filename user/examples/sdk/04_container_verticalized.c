...
int main(void)
{
    ...
    rc = yai_sdk_ws_status(client, &reply);
    print_step("ws status", rc, &reply);
    yai_sdk_reply_free(&reply);

    rc = yai_sdk_ws_graph_summary(client, &reply);
    print_step("ws graph summary", rc, &reply);
    yai_sdk_reply_free(&reply);

    rc = yai_sdk_ws_db_status(client, &reply);
    print_step("ws db status", rc, &reply);
    yai_sdk_reply_free(&reply);

    rc = yai_sdk_ws_data_evidence(client, &reply);
    print_step("ws data evidence", rc, &reply);
    yai_sdk_reply_free(&reply);

    rc = yai_sdk_ws_knowledge_status(client, &reply);
    print_step("ws knowledge status", rc, &reply);
    yai_sdk_reply_free(&reply);

    rc = yai_sdk_ws_policy_effective(client, &reply);
    print_step("ws policy effective", rc, &reply);
    yai_sdk_reply_free(&reply);

    rc = yai_sdk_ws_debug_resolution(client, &reply);

...
static int load_governable_objects(cJSON **root_doc, cJSON **objects_arr) {
  ...
  *root_doc = NULL;
  *objects_arr = NULL;
  if (snprintf(path, sizeof(path), "%s/registry/governable-objects.v1.json", resolve_law_root()) <= 0) return 2;
...
static int cmd_law_list(int argc, char **argv) {
  ...
  printf("  Source            %s/registry/governable-objects.v1.json\n", resolve_law_root());
...
static int run_govern_surface(int argc, char **argv) {
  ...
  if (snprintf(cli_path, sizeof(cli_path), "%s/tools/bin/yai-govern", resolve_law_root()) <= 0) {
    fprintf(stderr, "ERR: unable to resolve governance cli path\n");
    return 2;
  }
  if (access(cli_path, X_OK) != 0) {
    fprintf(stderr, "ERR: governance cli not executable: %s\n", cli_path);
  ...
  if (pid < 0) {
    free(child_argv);
    fprintf(stderr, "ERR: cannot fork governance cli\n");
    return 5;
  }
  if (pid == 0) {
  ...
  if (waitpid(pid, &status, 0) < 0) {
    fprintf(stderr, "ERR: governance cli wait failed\n");
    return 6;
  }
  if (WIFEXITED(status)) {
  ...
  if (WIFSIGNALED(status)) {
    fprintf(stderr, "ERR: governance cli terminated by signal %d\n", WTERMSIG(status));
    return 7;
  }
  return 7;
}
...
int yai_cmd_law(int argc, char** argv) {
  ...
  if (argc < 1) {
    ...
    printf("  yai law gov source import --path ingestion/sources\n");

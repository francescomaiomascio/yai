...
void yai_sdk_runtime_state_init(yai_sdk_runtime_state_t *out)
{
    ...
    out->workspace_binding = YAI_SDK_BINDING_UNKNOWN;
    ...
    out->knowledge.availability = YAI_SDK_AVAIL_UNKNOWN;
}

static void parse_workspace_binding(yai_sdk_runtime_state_t *out, const cJSON *data)
{
    ...
    copy_str(out->workspace_id, sizeof(out->workspace_id), ws_id);
    copy_str(out->workspace_alias, sizeof(out->workspace_alias), ws_alias);
    out->has_active_workspace = out->workspace_id[0] ? 1 : 0;

    if (cJSON_IsString(binding) && binding->valuestring) {
        if (streq(binding->valuestring, "active")) {
            out->workspace_binding = YAI_SDK_BINDING_ACTIVE;
        } else if (streq(binding->valuestring, "selected")) {
            out->workspace_binding = YAI_SDK_BINDING_SELECTED;
        } else if (streq(binding->valuestring, "degraded")) {
            out->workspace_binding = YAI_SDK_BINDING_DEGRADED;
        } else if (streq(binding->valuestring, "none") || streq(binding->valuestring, "detached")) {
            out->workspace_binding = YAI_SDK_BINDING_NONE;
        } else if (streq(binding->valuestring, "unbound")) {
            out->workspace_binding = YAI_SDK_BINDING_UNBOUND;
        }
    } else if (out->has_active_workspace) {
        out->workspace_binding = YAI_SDK_BINDING_SELECTED;
    }
}
...
static void set_families_from_command(yai_sdk_runtime_state_t *out, const cJSON *data)
{
    ...
    if (strstr(out->command_id, "workspace.run") != NULL) {
        out->exec.ready = 1;
        out->exec.bound = out->has_active_workspace;
    }

    if (strstr(out->command_id, "workspace.graph.summary") != NULL) {
        out->graph.ready = 1;
        out->graph.bound = out->has_active_workspace;
        out->knowledge.bound = out->has_active_workspace;
    ...
    if (cJSON_IsString(qf) && qf->valuestring) {
        if (streq(qf->valuestring, "graph")) {
            ...
            out->data.ready = 1;
            out->data.bound = out->has_active_workspace;
    ...
    out->knowledge.availability = avail_from_ready_degraded(out->knowledge.ready, out->knowledge.degraded);
}
...
int yai_sdk_runtime_state_from_reply_json(const char *exec_reply_json, yai_sdk_runtime_state_t *out)
{
    ...
    if (cJSON_IsObject(data)) {
        parse_workspace_binding(out, data);
        parse_recovery(out, data);
        set_families_from_command(out, data);
    }

    if (out->workspace_binding == YAI_SDK_BINDING_UNKNOWN) {
        if (out->has_active_workspace) {
            out->workspace_binding = YAI_SDK_BINDING_SELECTED;
        } else {
            out->workspace_binding = YAI_SDK_BINDING_NONE;
        }

...
int main(void)
{
  ...
  {
    char ws[64] = {0};
    if (yai_sdk_workspace_clear_binding() != 0)
    {
      fprintf(stderr, "sdk_smoke: context clear failed\n");
      return 1;
    }
    if (yai_sdk_workspace_current(ws, sizeof(ws)) != 1)
    {
      fprintf(stderr, "sdk_smoke: context should be empty after clear\n");
      return 1;
    }
    if (yai_sdk_workspace_bind("ws_smoke_ctx") != 0)
    {
      fprintf(stderr, "sdk_smoke: context set failed\n");
      return 1;
    }
    if (yai_sdk_workspace_current(ws, sizeof(ws)) != 0 || strcmp(ws, "ws_smoke_ctx") != 0)
    {
      fprintf(stderr, "sdk_smoke: context get mismatch\n");
      return 1;
    }
    memset(ws, 0, sizeof(ws));
    if (yai_sdk_workspace_resolve(NULL, ws, sizeof(ws)) != 0 || strcmp(ws, "ws_smoke_ctx") != 0)
    {

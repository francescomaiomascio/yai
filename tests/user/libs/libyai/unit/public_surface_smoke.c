...
int main(void)
{
  ...
  if (strcmp(YAI_SDK_FAMILY_EXEC, "exec") != 0 ||
      ...
      strcmp(YAI_SDK_FAMILY_KNOWLEDGE, "knowledge") != 0) {
    fprintf(stderr, "public_surface_smoke: family constants mismatch\n");
    return 1;
  }
  if (strcmp(YAI_SDK_CMD_RUNTIME_PING, "yai.runtime.ping") != 0 ||
      strcmp(YAI_SDK_EXEC_CMD_RUN_ACTION, "yai.workspace.run") != 0) {
    fprintf(stderr, "public_surface_smoke: command constants mismatch\n");
    return 1;
  }
  if (strcmp(YAI_SDK_GRAPH_CMD_LINEAGE, "yai.workspace.graph.lineage") != 0 ||
      strcmp(YAI_SDK_DB_CMD_TAIL, "yai.workspace.events.tail") != 0 ||
      strcmp(YAI_SDK_POLICY_CMD_EFFECTIVE, "yai.workspace.policy_effective") != 0 ||
      strcmp(YAI_SDK_RECOVERY_CMD_LOAD, "yai.workspace.lifecycle.maintain") != 0) {
    fprintf(stderr, "public_surface_smoke: workspace family constants mismatch\n");
    return 1;
  }

  {
    yai_sdk_reply_t out = {0};
    int rc_null = yai_sdk_ws_graph_summary(NULL, &out);
    if (rc_null != YAI_SDK_BAD_ARGS) {
      fprintf(stderr, "public_surface_smoke: expected BAD_ARGS for null client in typed helper, got %d\n", rc_null);
      return 1;
    }
    rc_null = yai_sdk_ws_policy_attach(NULL, "customer.default.org-workspace-contextual-review", &out);
    if (rc_null != YAI_SDK_BAD_ARGS) {
      fprintf(stderr, "public_surface_smoke: expected BAD_ARGS for null client in policy helper, got %d\n", rc_null);
      return 1;
    }
    rc_null = yai_sdk_ws_domain_set(NULL, "scientific", "parameter-governance", &out);
    if (rc_null != YAI_SDK_BAD_ARGS) {

...
int main(void)
{
  ...
  rc = yai_sdk_workspace_clear_binding();
  if (rc != 0)
  {
  ...
  rc = yai_sdk_workspace_resolve(NULL, ws, sizeof(ws));
  if (rc != 1)
  {
  ...
  {
    fprintf(stderr, "workspace_smoke: workspace id format failed\n");
    return 9;
  }

  rc = yai_sdk_workspace_bind(ws_id);
  if (rc != 0)
  {
  ...
  rc = yai_sdk_workspace_resolve("ws_override", ws, sizeof(ws));

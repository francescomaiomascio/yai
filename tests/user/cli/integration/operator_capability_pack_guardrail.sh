...
rg -n "^doctor workspace$|^BAD ARGS$|^Workspace status$" "$TMP/doctor_workspace.txt" >/dev/null

"$CLI" doctor all >"$TMP/doctor_all.txt" 2>&1 || true
rg -n "doctor|Runtime status|Workspace status|SERVER UNAVAILABLE|BAD ARGS" "$TMP/doctor_all.txt" >/dev/null

"$CLI" inspect workspace >"$TMP/inspect_workspace.txt" 2>&1 || true
rg -n "^inspect workspace$|^Workspace inspect$|^BAD ARGS$|^PROTOCOL ERROR$" "$TMP/inspect_workspace.txt" >/dev/null

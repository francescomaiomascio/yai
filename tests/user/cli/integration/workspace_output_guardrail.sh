...
if [[ "$RC_UP" -eq 40 ]]; then
  echo "workspace_output_guardrail: skipped (runtime unavailable)"
  exit 0
fi
...
rg -n "^Workspace set$" "$TMP/set.txt" >/dev/null
rg -n "^  Prompt token +.*${WS_ID}$" "$TMP/set.txt" >/dev/null
"$CLI" ws domain set --family economic --specialization payments >/dev/null
"$CLI" ws policy attach customer.default.org-workspace-contextual-review >"$TMP/attach.txt" 2>&1
rg -n "^Workspace policy attach$" "$TMP/attach.txt" >/dev/null
...
if [[ "$RC_GRAPH" -eq 0 ]]; then
  rg -n "^Workspace graph$|^Workspace query$" "$TMP/graph_summary.txt" >/dev/null
...
if [[ "$RC_DB_STATUS" -eq 0 ]]; then
  rg -n "^Workspace status$|^Workspace inspect$" "$TMP/db_status.txt" >/dev/null
else
...
if [[ "$RC_DATA_EVIDENCE" -eq 0 ]]; then
  rg -n "^Workspace data$|^Workspace query$" "$TMP/data_evidence.txt" >/dev/null
...
set +e
"$CLI" ws knowledge transient >"$TMP/knowledge_transient.txt" 2>&1
RC_KNOWLEDGE=$?
set -e
if [[ "$RC_KNOWLEDGE" -eq 0 ]]; then
  rg -n "^Workspace knowledge$|^Workspace query$|^Workspace inspect$" "$TMP/knowledge_transient.txt" >/dev/null
else
  rg -n "^(BAD ARGS|PROTOCOL ERROR|SERVER UNAVAILABLE)$" "$TMP/knowledge_transient.txt" >/dev/null
fi
...
rg -n "^  Workspace selected +yes$|^  Workspace bound +yes$" "$TMP/status_bound.txt" >/dev/null
rg -n "^  Exec +|^  Data +|^  Graph +|^  Knowledge +|^  Recovery state +" "$TMP/status_bound.txt" >/dev/null
...
if [[ "$RC_WS_INSPECT" -eq 0 ]]; then
  rg -n "^Workspace inspect$" "$TMP/ws_inspect.txt" >/dev/null
...
if [[ "$RC_INSPECT_RUNTIME" -eq 0 ]]; then
  ...
  rg -n "^Baseline$|^Workspace binding$|^Capability families$" "$TMP/inspect_runtime.txt" >/dev/null
  rg -n "^  Liveness +|^  Readiness +|^  Exec +|^  Data +|^  Graph +|^  Knowledge +" "$TMP/inspect_runtime.txt" >/dev/null
else
...
if [[ "$RC_POLICY" -eq 0 ]]; then
  rg -n "^Workspace policy effective$" "$TMP/policy.txt" >/dev/null
...
if [[ "$RC_DEBUG" -eq 0 ]]; then
  rg -n "^Workspace debug resolution$" "$TMP/debug.txt" >/dev/null
...
set -e
if [[ "$RC_RUN" -eq 0 ]]; then
  rg -n "^Workspace run$" "$TMP/run.txt" >/dev/null
  rg -n "^Execution$|^Outcome$|^Scientific$|^Digital$" "$TMP/run.txt" >/dev/null
else
  if rg -n "^Workspace run$" "$TMP/run.txt" >/dev/null; then
...
rg -n "^Workspace cleared$" "$TMP/clear.txt" >/dev/null
rg -n "^  Binding +(active|no_active)$" "$TMP/clear.txt" >/dev/null
"$CLI" ws policy detach customer.default.org-workspace-contextual-review >"$TMP/detach.txt" 2>&1 || true
"$CLI" ws unset >"$TMP/unset.txt" 2>&1 || true
rg -n "^Workspace unset$" "$TMP/unset.txt" >/dev/null
"$CLI" ws status >"$TMP/status_unbound.txt" 2>&1 || true
rg -n "^Workspace status$|^SERVER UNAVAILABLE$" "$TMP/status_unbound.txt" >/dev/null
if ! rg -n "^SERVER UNAVAILABLE$" "$TMP/status_unbound.txt" >/dev/null; then
  rg -n "^  Active +no$|^  Workspace selected +no$|^  Workspace bound +no$" "$TMP/status_unbound.txt" >/dev/null
fi
...
"$CLI" down >/dev/null 2>&1 || true
echo "workspace_output_guardrail: ok"

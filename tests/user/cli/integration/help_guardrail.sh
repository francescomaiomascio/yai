...
"$CLI" help ws knowledge >"$TMP/help_ws_knowledge.txt" 2>&1
rg -n "^ws knowledge$" "$TMP/help_ws_knowledge.txt" >/dev/null
rg -n "status|transient|memory|providers|context" "$TMP/help_ws_knowledge.txt" >/dev/null

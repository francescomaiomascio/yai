...
rg -n "^workspace status$|^workspace\\.status$|^workspace status$" "$TMP/ws_status.txt" >/dev/null || true
if [[ "$RC_WS_STATUS" -eq 0 ]]; then
  rg -n "^Workspace status$" "$TMP/ws_status.txt" >/dev/null
...
if [[ "$RC_WS_CURRENT" -eq 0 ]]; then
  rg -n "^Workspace current$" "$TMP/ws_current.txt" >/dev/null
...
if [[ "$RC_WS_RUN_NO_WS" -ne 20 && "$RC_WS_RUN_NO_WS" -ne 30 && "$RC_WS_RUN_NO_WS" -ne 40 ]]; then
  echo "expected ws run without active workspace to fail with rc=20/30/40, got $RC_WS_RUN_NO_WS"
  cat "$TMP/ws_run_no_ws.txt"
  exit 1
fi
if [[ "$RC_WS_RUN_NO_WS" -eq 20 ]]; then
  rg -n "^BAD ARGS$" "$TMP/ws_run_no_ws.txt" >/dev/null
  rg -n "No workspace selected|No active workspace binding for runtime execution|No active workspace selected for runtime execution" "$TMP/ws_run_no_ws.txt" >/dev/null

...
for f in doctor_runtime_unbound status_runtime_unbound inspect_runtime_unbound; do
  ...
  rg -n "^Baseline$|^Workspace binding$" "$TMP/$f.txt" >/dev/null
...
rg -n "^  Workspace selected +no$" "$TMP/doctor_runtime_unbound.txt" >/dev/null
rg -n "^  Workspace bound +no$" "$TMP/doctor_runtime_unbound.txt" >/dev/null
...
rg -n "^  Workspace selected +yes$" "$TMP/doctor_runtime_bound.txt" >/dev/null
rg -n "^  Workspace bound +yes$|^  Workspace bound +unknown$" "$TMP/doctor_runtime_bound.txt" >/dev/null
...
if [[ "$RC_GRAPH" -ne 20 ]]; then
  echo "expected ws graph summary rc 20 with no workspace, got $RC_GRAPH"

...
static void test_ws_current_maps_to_workspace_current(void)
{
    ...
    assert(rc == 0);
    assert(req.kind == YAI_PORCELAIN_KIND_COMMAND);
    assert(strcmp(req.command_id, "yai.workspace.current") == 0);
}
...
static void test_ws_status_maps_to_workspace_status(void)
{
    ...
    assert(rc == 0);
    assert(req.kind == YAI_PORCELAIN_KIND_COMMAND);
    assert(strcmp(req.command_id, "yai.workspace.status") == 0);
}
...
static void test_ws_domain_set_maps_to_workspace_domain_set(void)
{
    ...
    assert(rc == 0);
    assert(req.kind == YAI_PORCELAIN_KIND_COMMAND);
    assert(strcmp(req.command_id, "yai.workspace.domain_set") == 0);
}
...
static void test_ws_run_maps_to_workspace_run(void)
{
    ...
    assert(rc == 0);
    assert(req.kind == YAI_PORCELAIN_KIND_COMMAND);
    assert(strcmp(req.command_id, "yai.workspace.run") == 0);
...
static void test_ws_prompt_token_maps_to_prompt_context(void)
{
    ...
    assert(rc == 0);
    assert(req.kind == YAI_PORCELAIN_KIND_COMMAND);
    assert(strcmp(req.command_id, "yai.workspace.prompt_context") == 0);
}
...
static void test_ws_graph_summary_maps_to_workspace_graph_summary(void)
{
    ...
    assert(rc == 0);
    assert(req.kind == YAI_PORCELAIN_KIND_COMMAND);
    assert(strcmp(req.command_id, "yai.workspace.graph.summary") == 0);
}
...
static void test_ws_data_evidence_maps_to_workspace_query_family(void)
{
    ...
    assert(rc == 0);
    assert(req.kind == YAI_PORCELAIN_KIND_COMMAND);
    assert(strcmp(req.command_id, "yai.workspace.data.evidence") == 0);
...
static void test_ws_db_tail_maps_to_workspace_events_tail(void)
{
    ...
    assert(rc == 0);
    assert(req.kind == YAI_PORCELAIN_KIND_COMMAND);
    assert(strcmp(req.command_id, "yai.workspace.db.tail") == 0);
}

static void test_ws_knowledge_transient_maps_to_workspace_query(void)
{
    char *argv[] = {"yai", "ws", "knowledge", "transient", NULL};
    yai_porcelain_request_t req;
    int rc = yai_porcelain_parse_argv(4, argv, &req);
    assert(rc == 0);
    assert(req.kind == YAI_PORCELAIN_KIND_COMMAND);
    assert(strcmp(req.command_id, "yai.workspace.knowledge.transient") == 0);
...
static void test_ws_recovery_reopen_maps_to_workspace_open(void)
{
    ...
    assert(rc == 0);
    assert(req.kind == YAI_PORCELAIN_KIND_COMMAND);
    assert(strcmp(req.command_id, "yai.workspace.open") == 0);
}
...
static void test_ws_query_maps_to_workspace_query(void)
{
    ...
    assert(rc == 0);
    assert(req.kind == YAI_PORCELAIN_KIND_COMMAND);
    assert(strcmp(req.command_id, "yai.workspace.query") == 0);
}
...
static void test_inspect_mesh_projection_maps_to_workspace_graph_workspace(void)
{
    ...
    assert(rc == 0);
    assert(req.kind == YAI_PORCELAIN_KIND_COMMAND);
    assert(strcmp(req.command_id, "yai.workspace.graph.workspace") == 0);
}
...
static void test_inspect_grant_projection_maps_to_workspace_policy_effective(void)
{
    ...
    assert(rc == 0);
    assert(req.kind == YAI_PORCELAIN_KIND_COMMAND);
    assert(strcmp(req.command_id, "yai.workspace.policy_effective") == 0);
}
...
int main(void)
{
    ...
    test_ws_knowledge_transient_maps_to_workspace_query();

...
int main(void)
{
  ...
  const char *mind_bin = getenv("YAI_MIND_BIN");
  pid_t pid = fork();

  if (!mind_bin || !mind_bin[0]) {
    mind_bin = "./build/test/brain/bin/yai-mind-compat";
  }

  assert(pid >= 0);
  if (pid == 0) {
    execl(mind_bin, mind_bin, "--serve-once", "--socket", socket_path, (char *)NULL);
    _exit(127);
  }

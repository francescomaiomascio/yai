...
int yai_law_effective_stack_finalize(yai_law_effective_stack_t *stack) {
  ...
  if (stack->authority_contributor_count == 0) {
    (void)yai_law_safe_snprintf(stack->authority_contributors[0], sizeof(stack->authority_contributors[0]), "%s", "baseline_authority");
    stack->authority_contributor_count = 1;
  }
  if (stack->evidence_contributor_count == 0) {
    (void)yai_law_safe_snprintf(stack->evidence_contributors[0], sizeof(stack->evidence_contributors[0]), "%s", "resolution_trace");

                                   ...
                                   int64_t now_epoch)
{
  ...
  if (leader) {
    (void)snprintf(coordination->leader_peer_id, sizeof(coordination->leader_peer_id), "%s", leader);
  }

  (void)snprintf(coordination->mode,
                 sizeof(coordination->mode),
                 "%s",
                 participants > 1u ? "distributed" : "single_node");

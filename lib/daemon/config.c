...
int yai_daemon_config_set_string(char *dst, size_t dst_cap, const char *value)
{
...
static void apply_kv(yai_daemon_config_t *cfg, const char *key, const char *value)
{
  if (!cfg || !key || !value)
  {
    return;
  ...
  {
    (void)yai_daemon_config_set_string(cfg->home, sizeof(cfg->home), value);
  }
  else if (strcmp(key, "owner_ref") == 0)
  {
    (void)yai_daemon_config_set_string(cfg->owner_ref, sizeof(cfg->owner_ref), value);
  }
  else if (strcmp(key, "source_label") == 0)
  {
    (void)yai_daemon_config_set_string(cfg->source_label, sizeof(cfg->source_label), value);
  }
  else if (strcmp(key, "log_level") == 0)
  {
    (void)yai_daemon_config_set_string(cfg->log_level, sizeof(cfg->log_level), value);
  }
  else if (strcmp(key, "mode") == 0)
  {
    (void)yai_daemon_config_set_string(cfg->mode, sizeof(cfg->mode), value);
  }
  else if (strcmp(key, "bindings_manifest") == 0)
  {
    (void)yai_daemon_config_set_string(cfg->bindings_manifest, sizeof(cfg->bindings_manifest), value);
  }
  else if (strcmp(key, "tick_ms") == 0)
  {
    (void)yai_daemon_config_parse_uint(value, &cfg->tick_ms);
  }
  else if (strcmp(key, "max_ticks") == 0)
  {
    (void)yai_daemon_config_parse_int(value, &cfg->max_ticks);
  }
}
...
int yai_daemon_config_defaults(yai_daemon_config_t *cfg)
{
  ...
  (void)yai_daemon_config_set_string(cfg->log_level, sizeof(cfg->log_level), "info");
  (void)yai_daemon_config_set_string(cfg->mode, sizeof(cfg->mode), "foreground");
  cfg->tick_ms = 1000;
  cfg->max_ticks = 0;
  return 0;
}
...
int yai_daemon_config_apply_env(yai_daemon_config_t *cfg)
{
  ...
  if (raw && raw[0])
  {
    (void)yai_daemon_config_set_string(cfg->owner_ref, sizeof(cfg->owner_ref), raw);
  }
  raw = getenv("YAI_DAEMON_SOURCE_LABEL");
  if (raw && raw[0])
  {
    (void)yai_daemon_config_set_string(cfg->source_label, sizeof(cfg->source_label), raw);
  }
  raw = getenv("YAI_DAEMON_LOG_LEVEL");
  if (raw && raw[0])
  {
    (void)yai_daemon_config_set_string(cfg->log_level, sizeof(cfg->log_level), raw);
  }
  raw = getenv("YAI_DAEMON_MODE");
  if (raw && raw[0])
  {
    (void)yai_daemon_config_set_string(cfg->mode, sizeof(cfg->mode), raw);
  }
  ...
  if (raw && raw[0])
  {
    (void)yai_daemon_config_parse_uint(raw, &cfg->tick_ms);
  }

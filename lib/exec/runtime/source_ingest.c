                         ...
                         size_t reason_cap)
{
  ...
  cJSON_AddStringToObject(node, "state", "enrolled");
  ...
  cJSON_AddStringToObject(link, "registration_status", "registered");
  ...
  if (snprintf(out_json,
               out_cap,
               "{\"type\":\"yai.source.enroll.reply.v1\",\"workspace_id\":\"%s\",\"source_node_id\":\"%s\",\"daemon_instance_id\":\"%s\",\"owner_link_id\":\"%s\",\"registered\":true,\"mediation\":%s}",
               workspace_id,
               node_id,
               daemon_id,
               owner_link_id,
  ...
  (void)set_reason(reason, reason_cap, "source_enroll_accepted");
  return 0;
}

...
{
  puts("yai-daemon-manager - canonical daemon plane manager");
  printf("version: %s\n", YAI_VERSION_STRING);
  puts("");
  puts("usage: yai-daemon-manager [options]");
  puts("  --help                    show help");
  ...
  puts("  legacy compatibility: YAI_EDGE_*");
  puts("");
  puts("guardrails:");
  puts("  - yai-daemon-manager is subordinate to owner workspace sovereignty");
  puts("  - compatibility aliases `yai-daemon` and `yai-edge` remain available");
...
static int parse_args(int argc, char **argv, yai_edge_config_t *cfg)
{
  ...
  for (i = 1; i < argc; ++i)
  {
    ...
    fprintf(stderr, "yai-daemon-manager: unknown/invalid argument: %s\n", argv[i]);
    return -1;
  }
...
int main(int argc, char **argv)
{
  ...
  {
    fprintf(stderr, "yai-daemon-manager: failed to initialize defaults\n");
    return 70;
  }
  ...
  if (rc != 0)
  {
    fprintf(stderr, "yai-daemon-manager: config validation failed rc=%d\n", rc);
    return 78;
  }
  ...
  {
    fprintf(stderr,
            "yai-daemon-manager: background mode placeholder only; use --mode foreground in YD-2\n");
    return 78;
  }
  ...
  if (rc != 0)
  {
    fprintf(stderr, "yai-daemon-manager: runtime init failed rc=%d\n", rc);
    return 70;
  }
  if (yai_edge_lifecycle_install_signals() != 0)
  {
    fprintf(stderr, "yai-daemon-manager: failed to install signal handlers\n");
    return 70;
  }
  if (yai_edge_runtime_start(&rt) != 0)
  {
    fprintf(stderr, "yai-daemon-manager: runtime start failed\n");
    return 70;
  }
  ...
  if (rc != 0)
  {
    fprintf(stderr, "yai-daemon-manager: foreground loop failed rc=%d\n", rc);
  }

  if (yai_edge_runtime_shutdown(&rt) != 0)
  {
    fprintf(stderr, "yai-daemon-manager: shutdown failed\n");
    return 70;
  }

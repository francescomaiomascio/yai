...
static yai_episodic_record_t g_records[YAI_MIND_EPISODIC_CAP];
                                    ...
                                    const char *summary)
{
  ...
  if (!yai_node_id_is_valid(node_id)) {
    int rc = yai_graph_node_create("episodic", episode_id, summary, &node_id);
...
void yai_graph_episodic_reset(void)
{

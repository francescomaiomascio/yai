...
- Start infra cutover increment: add reusable `verify` workflow via `infra.1.0-rc1`, migrate governance guide pointer, and add `scripts/yai-govern` compatibility wrapper.
...
- Aligned cross-repo pins for `yai-specs` and `cli` on branch `meta/governance-proof-pack-lock` with canonical proof manifest updates.

...
Normative contract behavior (protocol/control/graph/vault/compliance) is defined in **`law` (canonical) and `embedded/law` (runtime contract surface)**. Contract issues may be triaged here but the source-of-truth fix may live in `law`.

...
SPEC="$ROOT/deps/yai-specs/vault/vault_abi.json"
...
DIFF_A=$(diff -u <(strip_generated < "$ROOT/deps/yai-specs/vault/yai_vault_abi.h") \
                 <(strip_generated < "$TMP_DIR/deps/yai-specs/vault/yai_vault_abi.h") || true)
if [[ -n "$DIFF_A" ]]; then
